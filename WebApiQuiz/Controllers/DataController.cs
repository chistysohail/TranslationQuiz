    ...
    public class DataController : ControllerBase
    {
        ...
        public IActionResult SearchMarkdown(string word)
        {
                ...
                for (int i = 0; i < lines1.Length; i++)
                {
                    ...
                    if (lines1[i].StartsWith("#") && lines1[i].TrimStart('#', ' ') == header)
                    {
                        ...
                        for (int j = i + 1; j < lines1.Length; j++)
                        {
                            ...
                            {
                                // If it's not, add it to the search results for this query
                                results2.File1Results.Add(lines1[j]);
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < lines2.Length; i++)
                {
                    ...
                    if (lines2[i].StartsWith("#") && lines2[i].TrimStart('#', ' ') == header)
                    {
                        ...
                        for (int j = i + 1; j < lines2.Length; j++)
                        {
                            ...
                            {
                                // If it's not, add it to the search results for this query
                                results2.File2Results.Add(lines2[j]);
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < lines3.Length; i++)
                {
                    ...
                    if (lines3[i].StartsWith("#") && lines3[i].TrimStart('#', ' ') == header)
                    {
                        ...
                        for (int j = i + 1; j < lines3.Length; j++)
                        {
                            ...
                            {
                                // If it's not, add it to the search results for this query
                                results2.File3Results.Add(lines3[j]);
                            }
                        }
                    }
                }
            ...
            if (searchResults.Count > 0)
            {
                ...
                // Loop through the search results and append them to the string builder
                foreach (SearchResults results3 in searchResults)
                {
                    // Check if any results were found for this query
                    if (results3.File1Results.Count > 0 || results3.File2Results.Count > 0 ||
                        results3.File3Results.Count > 0)
                    {
                        // Append the header for this query
                        sb.AppendLine($"Search results for '{results3.Header}':");

                        // Append the results for each file
                        if (results3.File1Results.Count > 0)
                        {
                            sb.AppendLine("Results from file 1:");
                            foreach (string line in results3.File1Results)
                            {
                                sb.AppendLine(line);
                            }
                        }

                        if (results3.File2Results.Count > 0)
                        {
                            sb.AppendLine("Results from file 2:");
                            foreach (string line in results3.File2Results)
                            {
                                sb.AppendLine(line);
                            }
                        }

                        if (results3.File3Results.Count > 0)
                        {
                            sb.AppendLine("Results from file 3:");
                            foreach (string line in results3.File3Results)
                            {
                                sb.AppendLine(line);
                            }
                        }
